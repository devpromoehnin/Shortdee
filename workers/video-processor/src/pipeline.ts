 ...
 *
 * Stages: preprocess → transcribe → detect moments → finalize.
 * Clip cutting (reframe / captions) is Phase 5 (Auto-Edit Pipeline).
 */
export async function runPipeline(job: Job<VideoJobData>): Promise<void> {
  ...
  try {
    ...
    const { audioPath, durationSec } = await preprocessStage(storageKey, workDir)
    await prisma.liveStream.update({
      where: { id: liveStreamId },
    ...
    log(`done — ${momentCount} moments detected`)
  } catch (err) {
